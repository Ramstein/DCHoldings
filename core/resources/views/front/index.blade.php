        ...
        <div class="row justify-content-lg-between mb-5">
            <div class="col-md-6 col-lg-6">
                ...
                <div class="mb-3">
                    ...
                    <a class="btn btn-link mb-2 mb-sm-0" href="{{route('contact')}}">Let's Talk <i
            ...
            <div class="row align-items-md-center">
                ...
                <div class="col-md-3 column-divider-md">
                    <div class="pl-md-2">
                        ...
                        <a class="font-size-1 font-weight-bold" href="{{route('user.savings')}}">{{__('Apply Now')}} <i
                                class="fal fa-angle-right fa-sm ml-1"></i></a>
                    </div>
                </div>
            </div>
        ...
        <div class="w-lg-100 mx-lg-auto position-relative">
            <div class="text-center">
                <ul class="nav nav-segment nav-pills scrollbar-horizontal mb-7" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link @if($loop->first) active @endif" id="pills-{{$val->id}}-code-features-tab"
                            data-toggle="pill" href="#pills-{{$val->id}}-code-features" role="tab"
                            aria-controls="pills-{{$val->id}}-code-features" aria-selected="true">{{$val->name}}</a>
                    </li>
                    @endforeach
                </ul>
            </div>
            <div class="tab-content pr-lg-4">
                @foreach($category as $val)
                <div class="tab-pane fade @if($loop->first)show active @endif" id="pills-{{$val->id}}-code-features"
                    role="tabpanel" aria-labelledby="pills-{{$val->id}}-code-features-tab">
                            ...
                            <div class="card h-100">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="media font-size-1 mb-3">
                                        ...
                                        <div class="media-body">
                                            {{$currency->symbol.number_format($val->min_deposit)}}
                                            {{__('Minimum Deposit')}}
                                        </div>
                                    </div>
                                    <div class="media font-size-1 mb-3">
                                    ...
                                    <div class="media font-size-1 mb-3">
                                        <i class="fal fa-check text-success mt-1 mr-2"></i>
                                        <div class="media-body">
                                            @if($val->ref_percent!=null){{$val->ref_percent}}% @else {{__('No')}}
                                            @endif{{__('Referral Bonus')}}
                                        </div>
                                    </div>
                                    <div class="media font-size-1 mb-3">
                                        <i class="fal fa-check text-success mt-1 mr-2"></i>
                                        <div class="media-body">
                                            @if($val->bonus!=null){{$val->bonus}}% @else {{__('No')}}
                                            @endif{{__('Investment Bonus')}}
                                        </div>
                                    </div>
                                    <div class="media font-size-1 mb-3">
                                        <i class="fal fa-check text-success mt-1 mr-2"></i>
                                        <div class="media-body">
                                            @if($val->claim==1) {{__('Access to Profit anytime')}} @else
                                            {{__('Access to profit at end of plan')}} @endif
                                        </div>
                                    </div>
                                    <div class="media font-size-1 mb-3">
                                        <i class="fal fa-check text-success mt-1 mr-2"></i>
                                        <div class="media-body">
                                            @if($val->recurring==1) {{__('Recurring capital investment')}} @else
                                            {{__('No recurring capital investment')}} @endif
                                        </div>
                                    </div>
                                </div>
                                <div class="card-footer border-0">
                                    <a href="{{route('register')}}"
                                        class="btn btn-soft-primary btn-block transition-3d-hover">{{__('Get Started')}}</a>
                                </div>
                            </div>
                        </div>
                        @endforeach
    ...
    <div class="container-xl container-fluid space-1 space-md-2 px-4 px-md-8 px-lg-10">
        <div class="px-3">
            ...
            <div class="w-md-80 w-lg-50 text-center mx-md-auto mb-5 mb-md-9">
                <span
                    class="d-block small text-light font-weight-bold text-cap mb-2">{{__('INVEST IN COMPANIES')}}</span>
                <h2 class="text-light">{{__('See how')}} {{$set->site_name}}
                    {{__('is helping people get organized and work smarter')}}</h2>
            </div>
            <!-- End Title -->
            <!-- Nav -->
            <div class="text-center">
                            ...
                            <a href="{{route('check.plan', ['id' => $val->slug])}}">
                                <div class="card h-100">
                                    <div class="card-body">
                                        ...
                                        <small class="card-text"><span class="text-success">{{$val->interest}}%</span>
                                            {{__('Returns in')}} {{$val->duration.' '.$val->period}}</small>
                                        <small class="card-text"><span
                                                class="text-success">{{$currency->symbol.$val->price}}</span>
                                            {{__('per Unit')}}</small>
                                    </div>
                                </div>
                            </a>
                    ...
                    <div class="card-footer border-0 bg-transparent pt-0 px-5 pb-5">
                        <div class="media align-items-center">
                            <div class="avatar avatar-circle mr-3">
                                <img class="avatar-img" src="{{url('/')}}/asset/review/{{$val->image_link}}"
                                    alt="Image Description">
                            </div>
                            <div class="media-body">
        ...
        <div class="col-md-4 mb-7 mb-lg-0">
            <div data-aos="fade-up" data-aos-delay="100" class="aos-init aos-animate">
                ...
                <div class="text-center px-md-3 px-lg-7">
                    ...
                    <span class="text-dark font-weight-bold">{{$currency->symbol.number_format($t_profit)}}</span>
                    <p class="mb-0">{{__('Profit Shared')}}</p>
                </div>
                <!-- End Stats -->
            </div>
        </div>
                    ...
                    <figure class="mb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 71.7 64" width="71"
                            ...
                            <g transform="matrix(1 0 0 1 0 1.907349e-06)" style="clip-path:url(#SVGID_2_);">
                                <image width="100" height="100" xlink:href="{{url('/')}}/asset/images/img9.jpg"
                            ...
                            <g style="clip-path:url(#SVGID_4_);">
                                <image width="100" height="100" xlink:href="{{url('/')}}/asset/images/img3.jpg"
                            ...
                            <g style="clip-path:url(#SVGID_6_);">
                                <image width="100" height="100" xlink:href="{{url('/')}}/asset/images/img8.jpg"
                                    transform="matrix(0.3771 0 0 0.3771 2 24.2857)"></image>
                            </g>
                            <use xlink:href="#SVGID_5_" fill="none" stroke="#FFFFFF" stroke-width="4"></use>
                        </svg>
                    </figure>
                    <span class="text-dark font-weight-bold">{{$currency->symbol.number_format($t_amount)}}</span>
                    <p class=" mb-0">{{__('Money invested')}}</p>
                </div>
                <!-- End Stats -->
            </div>
        </div>
        ...
        <div class="col-md-4">
            <div data-aos="fade-up" data-aos-delay="100" class="aos-init aos-animate">
                ...
                <div class="text-center column-divider-md column-divider-20deg px-md-3 px-lg-7">
                    ...
                    <span class="text-dark font-weight-bold">{{$currency->symbol.number_format($t_payout)}}</span>
                    <p class=" mb-0">{{__('Payouts processed')}}</p>
                </div>
                <!-- End Stats -->
            </div>
        </div>
...
<div class="container space-2">
    ...
    <div class="js-slick-carousel slick slick-gutters-3 mb-5 mb-lg-3" data-hs-slick-carousel-options='{
        ...
        <div class="js-slide pb-6">
            <!-- Team -->
            <img class="img-fluid w-100 rounded" src="{{url('/')}}/asset/review/{{$val->image}}"
                alt="Image Description">
