...
//Fund account 
    ...
    public function fundsubmit(Request $request)
    {
        ...
        if ($gate->minamo <= $request->amount && $gate->maxamo >= $request->amount) {
            $charge = ($request->amount*$gate->percent_charge/100)+$gate->fiat_charge;
            ...
            if($gate->type==1){
                ...
                if($set['email_notify']==1){
                    send_email($user->email,$user->username,'Deposit request under review','We are currently reviewing your deposit of '.$request->amount.$currency->name.', once confirmed your balance will be credited automatically.<br>Thanks for working with us.');    			
                    send_email($set->email,$set->site_name,'New manual deposit request','Hello admin, you have a new manual deposit request for '.$token);
                }
            }
            ...
            if($gate->type==0){
                ...
                return back()->with('success', 'Deposit is under review');
            }   
        } else {
...
//Withdrawal
    ...
    public function withdrawsubmit(Request $request)
    {
        ...
        $pieces = explode("*", $request->coin);
        $ver=Withdrawm::whereid($pieces[0])->first();
        if($set->ns==1){
            $due=$set->next_settlement;
        }else{
            $dt = Carbon::now();
            $dt->add($ver->period.' '.$ver->duration); 
            $due=$dt;
        }
        if($request->type==1){
            if($user->profit>$amount || $user->profit==$amount){
                $sav= new Withdraw();
                $sav->user_id=Auth::guard('user')->user()->id;
                $sav->amount=$amount-($request->amount*$request->percent_charge/100+($request->fiat_charge));
                $sav->charge=$request->amount*$request->percent_charge/100+($request->fiat_charge);
                $sav->status=0;
                $sav->details=$request->details;
                $sav->type=$request->type;
                $sav->coin_id=$pieces[0];
                $sav->reference=$token;
                $sav->next_settlement=$due;
                $sav->save();
                $audit['user_id']=Auth::guard('user')->user()->id;
                $audit['trx']=str_random(16);
                ...
                $user->save();
                if($set->email_notify==1){
                    custom_email('new_withdraw_request_user', $sav->id);            
                    custom_email('new_withdraw_request_admin', $sav->id);            
                }
            ...
            if($user->balance>$amount || $user->balance==$amount){
                ...
                $sav->reference=$token;
                $sav->next_settlement=$due;
                $sav->duration=$ver->period.' '.$ver->duration;
                $sav->save();
                $audit['user_id']=Auth::guard('user')->user()->id;
                $audit['trx']=str_random(16);
                $audit['log']='Submitted withdraw request'.$token;
        ...
        if($set->transfer==1){
            ...
            if($user->merchant_id!=$request->merchant_id){
                ...
                if($bal>$amount || $bal==$amount){
                    $check=User::wheremerchant_id($request->merchant_id)->get();
                    if(count($check)>0){
                        $trans=User::wheremerchant_id($request->merchant_id)->first();
                        $token=str_random(16);
                        $sav['ref_id']=$token;
                        $sav['amount']=$request->amount;
                        ...
                        return back()->with('alert', 'Invalid merchant ID');
                    }
                }else{
...
//Settings
    public function delaccount(Request $request)
    {
        ...
        $earning = Earning::whereref_id($id)->delete();
...
//Plans
    ...
    public function check_plan(Request $request)
    {
        ...
        if(!empty($request->coupon)){
            $check=Coupon::where('code',$request->code)->where('status', 1)->count();
            if($check==1){
        ...
        if($bal>$main || $bal==$main){
            if($min_check>$plan->min_deposit || $min_check==$plan->min_deposit){
                if($amount_check<$plan->amount  || $amount_check==$plan->amount){
                    ...
                    $sav['profit']=0;
                    $sav['type']=$request->type;
                    if($set->instant_approval==0){
                        $sav['status']=3;
                    }elseif($set->instant_approval==1){
                        $sav['status']=1;
                    }
                    $sav['end_date']=$ndate;
                    $sav['date']=$date;
                    ...
                    Audit::create($audit);
                    if($request->type==1){
                        $user->profit=$user->profit-$main;
                    }elseif($request->type==2){
                        $user->balance=$user->balance-$main;
                    }elseif($request->type==3){
                        $user->ref_bonus=$user->ref_bonus-$main;
                    }
                    $user->save();
                    ...
                    if($set['email_notify']==1){
                    send_email($user->email, $user->site_name, 'Investment has started', '#'.$plan->trx.' will be updated daily and run for '.$plan->duration.' '.$plan->period.'. Thanks for choosing us.');
                    }
...
//Project
    public function sandtrades()
    {
        ...
        $data['sharing']=Sandprofits::wheresender_id(Auth::guard('user')->user()->id)->orderby('id', 'DESC')->get();
    ...
    public function sandcheck_plan(Request $request)
    {
        ...
        if($bal>$amount || $user->balance==$amount){
            $sav['user_id']=Auth::guard('user')->user()->id;
            $sav['plan_id']=$plan->id;
    ...
    public function sandshare_plan(Request $request)
    {
        $currency=Currency::whereStatus(1)->first();
        $set=Settings::find(1);
        $profit=Sandprofits::wheretrx($request->trx)->first();
        $plan=Sandplans::whereid($profit->plan_id)->first();
        $check=User::wheremerchant_id($request->merchant_id)->count();
        if($check==1){
            if($request->merchant_id==Auth::guard('user')->user()->merchant_id){
                return back()->with('alert', 'You can\'t share units with your self');
            }else{
                $token='PRO-'.str_random(6);
                $merchant=User::wheremerchant_id($request->merchant_id)->first();
                $amount=$plan->price*$request->units;
                $sav=new Sandprofits();
                $sav->user_id=$merchant->id;
                $sav->sender_id=Auth::guard('user')->user()->id;
                $sav->plan_id=$plan->id;
                $sav->units=$request->units;
                $sav->amount=$amount;
                ...
                $sav->status=1;
                $sav->trx=$token;
                $sav->expiring_date = $profit->expiring_date;
                $sav->save();
                $final=$profit->units-$request->units;
                if($final==0){
                    $profit->delete();
                }else{
                    $profit->units=$profit->units-$request->units;
                    $profit->amount=$plan->price*$final;
                    $profit->profit=($plan->price*$final)+($plan->price*$final*$plan->interest/100);
                    $profit->save();
                }
                $audit['user_id']=Auth::guard('user')->user()->id;
                $audit['trx']=$token;
                $audit['log']='Shared '.$request->units.' from '.$profit->trx.' with '.$merchant->id;
                Audit::create($audit);
    ...
    {
        $plan=Sandplans::whereslug($id)->first();
        $data = Sandfollowed::whereplan_id($plan->id)->whereuser_id(Auth::guard('user')->user()->id)->first();
        $data->delete();
