...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        View::composer('*', function($view){
            ...
            $set=Settings::first();
            $lang = Language::wherestatus(1)->get();
            if($set->ns==1){
            ...
            $profit=$data['profit']=Profits::wherestatus(1)->get();
            foreach($profit as $xpro){
                ...
                if (Carbon::now()<$ndate){
                    ...
                    if($xpro->status==1){
                        $profits->status=2;
                        $profits->save();
                        $amount=$xpro->real_profit-$profits->claimed;
                        //Profit
                        $user->balance=$user->balance+$profits->amount;
                    ...
                    if(Carbon::create($val->end_date)->lessThanOrEqualTo(Carbon::now())){
                        ...
                        if($set->email_notify==1){
                            send_email(
                                ...
                                'Savings of '.$val->profit.$currency->name.'. is now available in your account.<br>Thanks for working with us.'
                            );
                        }                 
                    } 
                }
            ...
            $pprofit=Profits::where('status', 3)->get();
            $view->with('pwithdraw', $pwithdraw );
            $view->with('pprofit', $pprofit );
